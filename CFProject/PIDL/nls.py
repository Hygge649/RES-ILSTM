...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, v0, tb, X_f, layers, lb, ub):
        ...
        self.t_lb = X_lb[:, 1:2]   #tb

        self.x_ub = X_ub[:, 0:1]  #ub[0]
        self.t_ub = X_ub[:, 1:2]  #tb

        self.x_f = X_f[:, 0:1]  #collocation_state
        self.t_f = X_f[:, 1:2]  #collocation_action
        ...
        self.optimizer = tfp.optimizer.lbfgs_minimize
            # tfp.optimizer.lbfgs_minimize(self.loss,
                                                      #initial_position=tf.reshape([-5.0, 0.0], [1, -1]))
                                                      # tf.convert_to_tensor(np.array([-5.0, 0.0])))
                                                    #self.loss,
                                                    # initial_position=lb,
                                                    # max_iterations=500,)
    ...
    def initialize_NN(self, layers):
        ...
        for l in range(0, num_layers - 1): #l = 0,1,2
    ...
    #
    # def __call__(self, inputs, weights, biases):
...
if __name__ == "__main__":
    ...
    t = data['tt'].flatten()[:, None] #time
    x = data['x'].flatten()[:, None] #position
    Exact = data['uu']
