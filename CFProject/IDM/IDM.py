...
# 403641.13072
#a v b s t
# a_max_n_boundary=[0.1, 2.5], desired_V_n_boundary=[1, 40], a_comf_n_boundary=[0.1, 5],
#S_jam_boundary=[0.1, 10], desired_T_n_boundary=[0.1, 5],
lb_x_idm = [0.1, 1, 0.1, 0.1, 0.1]
ub_x_idm = [2.5, 40, 5, 10, 5]
# 6.573689  167.608462  pso
# 34.259765  902.362158   ga
#19.65403  568.546701   de
...
def function_idm(p):
    ...
    follower_position = train["LocalY"].values
    follower_speed = train["Mean_Speed"].values
    speed_diff = train["speed_diff"].values
    space_headway = train["gap"].values  # gap, v.diff
    follower_acc = train["Mean_Acceleration"].values
...
arr = np.zeros((1, 4))
idm_error = pd.DataFrame(arr, columns=['MSE_V', 'MSE_GAP', 'RMSE_V', 'RMSE_GAP'], index=['train'])
...
for filefoldname in os.listdir(filefoldnames):
    ...
    path = os.path.join(filefold, file[-3])
    train = pd.read_csv(path, delim_whitespace=True, encoding='utf-8')

    pso_idm = PSO(func=function_idm, dim=5, pop=50, max_iter=50, lb=lb_x_idm, ub=ub_x_idm, w=0.8, c1=0.5, c2=0.5)
    best_x_pso, best_y_pso = pso_idm.run()

    # ga_idm = GA(func=function_idm, n_dim=5, size_pop=50, max_iter=20, lb=lb_x_idm, ub=ub_x_idm)
    # best_x_ga, best_y_ga = ga_idm.run()

    # de_idm = DE(func=function_idm, n_dim=5, size_pop=20, max_iter=20, lb=lb_x_idm, ub=ub_x_idm)
    # best_x_de, best_y_de = de_idm.run()

    a, v, x = draw_picture_idm(best_x_pso, train)
    # a, v, x = draw_picture_idm(best_x_ga, train)
    # a, v, x = draw_picture_idm(best_x_de, train)
    s = train["LocalY_leader"] - train["Vehicle_length"] - x

    IDM_TRAIN = output(a, v, x, s, train)
    # train_path = 'idm.txt'
    # train_path = os.path.join(filefold, train_path)
    # IDM_TRAIN.to_csv(train_path, sep='\t', index=False)

    final_picture(train, a, v, x, s)

    #return fixed para and loss(RMSE_ACC) for one driver

    cur_para = []
    cur_para.extend(best_x_pso)
    cur_para.extend(best_y_pso)
    # cur_para.extend(best_x_ga)
    # cur_para.extend(best_y_ga)
    # cur_para.extend(best_x_de)
    # cur_para.extend(best_y_de)
    cur_para = np.array(cur_para)
    cur_para = pd.DataFrame(cur_para.reshape(1, 6),
                            columns=['Maximum_Acc',  'Desire_Spe', 'Comfortable_Dec',
                                     'Minimum_Spa',  'Desire_Spa_Tim', 'loss(RMSE_ACC)'])

    cur_path = 'using_all_data.txt'
